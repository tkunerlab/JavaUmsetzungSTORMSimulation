 ...
 * @brief Sketch of GUI
 * 
 *        This is the main program that implements both plotter and calculator
 *        for STORM simulation. It can load new data from the editor or parse
...
public class Gui extends JFrame implements TableModelListener, PropertyChangeListener, ThreadCompleteListener {
	...
	private JLabel loadDataLabel = new JLabel("");
	...
	public Gui() {
		...
		try {
			tray.add(new TrayIcon(new ImageIcon(getClass().getResource("/resources/LogoCropped.png")).getImage(),
					"icon"));
		...
		scrollPane_1.setViewportBorder(
				new TitledBorder(null, "ViewPort", TitledBorder.LEADING, TitledBorder.TOP, null, null));
		...
		verticalBox_1.setBorder(new TitledBorder(UIManager.getBorder("TitledBorder.border"), "Structural Parameters",
				TitledBorder.LEADING, TitledBorder.TOP, usedFont, null));
		...
		verticalBox_2.setBorder(
				new TitledBorder(UIManager.getBorder("TitledBorder.border"), "Label And Structure Dependent Parameters",
		...
		verticalBox_3.setBorder(new TitledBorder(UIManager.getBorder("TitledBorder.border"), "Acquisition Parameters",
				TitledBorder.LEADING, TitledBorder.TOP, null, new Color(0, 0, 0)));
		...
		verticalBox_10.setBorder(new TitledBorder(UIManager.getBorder("TitledBorder.border"),
				"Label Dependent Parameters", TitledBorder.LEADING, TitledBorder.TOP, null, new Color(0, 0, 0)));
		verticalBox_9.add(verticalBox_10);

		Box horizontalBox_5 = Box.createHorizontalBox();
		verticalBox_10.add(horizontalBox_5);

		JLabel lblMeanAngle = new JLabel("Binding Angle [\u00B0]");
		...
		bleachConstantField.setAlignmentX(Component.LEFT_ALIGNMENT);
		...
		verticalBox_21.setBorder(new TitledBorder(UIManager.getBorder("TitledBorder.border"), "PSF Parameters",
				TitledBorder.LEADING, TitledBorder.TOP, null, new Color(0, 0, 0)));
		...
		minIntensityField.getDocument().addDocumentListener(new MyDocumentListener(minIntensityField));
		...
		JLabel lblMeanBlinkingDuration = new JLabel("Mean Blinking Duration [s]");
		...
		meanBlinkingDurationField.getDocument().addDocumentListener(new MyDocumentListener(meanBlinkingDurationField));
		...
		verticalBox_20.setBorder(
				new TitledBorder(null, "Camera Parameters", TitledBorder.LEADING, TitledBorder.TOP, null, null));
		...
		pxToNmField.getDocument().addDocumentListener(new MyDocumentListener(pxToNmField));
		...
		JLabel lblNewLabel_10 = new JLabel("<html>Frame Rate [s<sup>-1</sup>]</html>");
		...
		framerateField.getDocument().addDocumentListener(new MyDocumentListener(framerateField));
		...
		JLabel lblSigmaBackground = new JLabel("Readout Noise [DN]");
		...
		sigmaBgField.getDocument().addDocumentListener(new MyDocumentListener(sigmaBgField));
		...
		constOffsetField.getDocument().addDocumentListener(new MyDocumentListener(constOffsetField));
		...
		emGainField.getDocument().addDocumentListener(new MyDocumentListener(emGainField));
		...
		quantumEfficiencyField.getDocument().addDocumentListener(new MyDocumentListener(quantumEfficiencyField));
		...
		electronsPerDnField.getDocument().addDocumentListener(new MyDocumentListener(electronsPerDnField));
		...
		verticalBox_12.setBorder(
				new TitledBorder(null, "Rendering Parameters", TitledBorder.LEADING, TitledBorder.TOP, null, null));
		verticalBox_19.add(verticalBox_12);

		Box horizontalBox_54 = Box.createHorizontalBox();
		verticalBox_12.add(horizontalBox_54);

		JLabel lblWindowsizePsfRendering_1 = new JLabel("Windowsize PSF Rendering [px]");
		...
		windowsizePSFRenderingField.getDocument()
				.addDocumentListener(new MyDocumentListener(windowsizePSFRenderingField));
		...
		JLabel lblEmptyPixelsOn = new JLabel("Empty Pixels On Rim [px]");
		...
		emptyPixelsOnRimField.getDocument().addDocumentListener(new MyDocumentListener(emptyPixelsOnRimField));
		...
		naField.getDocument().addDocumentListener(new MyDocumentListener(naField));
		...
		wavelengthField.getDocument().addDocumentListener(new MyDocumentListener(wavelengthField));
		...
		JLabel lblDefokus = new JLabel("Defocus [nm]");
		...
		defokusField.getDocument().addDocumentListener(new MyDocumentListener(defokusField));
		...
		JLabel lblFokus = new JLabel("Focus [nm]");
		...
		fokusField.getDocument().addDocumentListener(new MyDocumentListener(fokusField));
		...
		verticalBox_22
				.setBorder(new TitledBorder(null, "PSF Settings", TitledBorder.LEADING, TitledBorder.TOP, null, null));
		...
		distributePSFOverFrames = new JCheckBox("Distribute PSFs");
		horizontalBox_55.add(distributePSFOverFrames);

		JButton exportTiffStackButton = new JButton("Export Tiff Stack");
			...
			public void actionPerformed(ActionEvent e) {
				if (allDataSets.size() > 0) {
					...
					if (returnValue == JFileChooser.APPROVE_OPTION) {
						...
						CreateTiffStack cts = new CreateTiffStack(allDataSets.get(currentRow), path, random,
								selfReference);
...
//							set.getConstOffset(), set.getCalibrationFile(), path,set.isEnsureSinglePSF(), set.isDistributePSFoverFrames());
					...
					JOptionPane.showMessageDialog(null, "Error: Please import an example or file first.",
		...
		calcButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (allDataSets.size() > 0) {
					...
					JOptionPane.showMessageDialog(null, "Error: Please import a file or example first.",
							"Error Massage", JOptionPane.ERROR_MESSAGE);
				}
			}
		});
		...
		tabbedPane_2.addTab("Visualization Parameters", null, verticalBox_13, null);
		...
		verticalBox_6.setBorder(new TitledBorder(null, "Visualization Parameter", TitledBorder.LEADING,
				TitledBorder.TOP, usedFont, null));
		...
		antibodyColorButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				allDataSets.get(currentRow).getParameterSet()
						.setAntibodyColor(JColorChooser.showDialog(getContentPane(), "Choose color for antibodies",
								allDataSets.get(currentRow).getParameterSet().getAntibodyColor()));
				updateButtonColors();
			}
		});
		...
		JLabel lblShowEm = new JLabel("Show Model");
		...
		emColorButton.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				allDataSets.get(currentRow).getParameterSet().setEmColor(JColorChooser.showDialog(getContentPane(),
						"Choose color for EM", allDataSets.get(currentRow).getParameterSet().getEmColor()));
				updateButtonColors();
			}
		});
		...
		backgroundColorButton.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				backgroundColor = JColorChooser.showDialog(getContentPane(), "Choose color for background",
						backgroundColor);
				// backgroundColor = new org.jzy3d.colors.Color(chosenColor.getRed(),
		...
		xminField.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				allDataSets.get(currentRow).getParameterSet().getBorders().set(0,
		...
		xmaxField.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				allDataSets.get(currentRow).getParameterSet().getBorders().set(1,
		...
		yminField.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				allDataSets.get(currentRow).getParameterSet().getBorders().set(2,
		...
		ymaxField.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				allDataSets.get(currentRow).getParameterSet().getBorders().set(3,
		...
		zminField.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				allDataSets.get(currentRow).getParameterSet().getBorders().set(4,
						Float.parseFloat(zminField.getText()));
			}
		...
		zmaxField.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				allDataSets.get(currentRow).getParameterSet().getBorders().set(5,
						Float.parseFloat(zmaxField.getText()));
			}
		...
		stormColorButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				allDataSets.get(currentRow).getParameterSet()
						.setStormColor(JColorChooser.showDialog(getContentPane(), "Choose color for Storm points",
								allDataSets.get(currentRow).getParameterSet().getStormColor()));
				updateButtonColors();
			}
		});
...
//				t.start();
		...
		plotPanel.add(loadDataLabel, "name_18510610515392");
		...
		JButton btnEvaluate = new JButton("Evaluate Reconstruction");
	...
	/**
	 * Configures the mouse listener for the dataset table. When a line is clicked,
	 * its ParameterSet is loaded to the configuration panel.
	 */
	...
	/**
	 * invoked by calculate button runs the calculation for the current dataset
	 * 
	 * @throws Exception
	 */
	...
	void calculate(DataSet currDataSet, boolean forTiffStack) {
		...
		if (forTiffStack) {
			...
			currDataSet.getParameterSet().setSz(0.0001f); // set localization precision to 0 to get the fluorophore
															// centers instead of STORM localizations
		}
...
//	}
	...
	/**
	 * sets the plot quality according to the radiobuttons, removed since there
	 * occured rendering errors for nicest and advanced
	 */
	...
	/**
	 * // * Checks which data sets are generally visible and creates a new Plot3D
	 * with the dataSets //
	 */
...
//	
		...
		Runnable warn = new Runnable() {
			...
			public void run() {
				try {
					...
					JOptionPane.showMessageDialog(null, "Error: Please enter a valid number", "Error Massage",
	...
	public void copyFields() {
		allDataSets.get(currentRow).getParameterSet()
				.setPabs((float) (new Float(labelingEfficiencyField.getText()) / 100.));
		allDataSets.get(currentRow).getParameterSet()
				.setAoa((float) ((new Float(meanAngleField.getText())) / 180 * Math.PI));
		allDataSets.get(currentRow).getParameterSet()
				.setSoa((float) ((new Float(angularDistributionField.getText())) / 180 * Math.PI));
		allDataSets.get(currentRow).getParameterSet()
				.setDeff((float) (new Float(detectionEfficiencyField.getText()) / 100));
		...
		allDataSets.get(currentRow).getParameterSet()
				.setMeanPhotonNumber((new Float(averagePhotonOutputField.getText())).intValue());
		...
		allDataSets.get(currentRow).getParameterSet()
				.setWindowsizePSF(new Integer(windowsizePSFRenderingField.getText()));
		...
		allDataSets.get(currentRow).getParameterSet()
				.setMeanBlinkingTime(new Float(meanBlinkingDurationField.getText()));
